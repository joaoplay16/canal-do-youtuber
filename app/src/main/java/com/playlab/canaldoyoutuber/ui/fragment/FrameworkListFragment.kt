...
import com.playlab.canaldoyoutuber.databinding.FragmentFrameworkListBinding
...
abstract class FrameworkListFragment : Fragment() {
    ...
    private lateinit var binding: FragmentFrameworkListBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentFrameworkListBinding.inflate(inflater)
        return binding.root
    }
    ...
    protected fun setUiModel(
        ...
        binding.tvMainText.text = titleText
        binding.tvSubTitle.text = subTitleText
        binding.tvSubTitle.visibility = if( subTitleText.isEmpty() ){
            View.GONE
        }
        else{
    ...
    private fun updateMainTextMargin(){
        val tvTitleLayoutParams = (binding.tvMainText.layoutParams as LinearLayout.LayoutParams)
    ...
    protected fun callExternalApp(
         ...
         */
        if( intentApp != null
            && intentApp.resolveActivity( requireActivity().packageManager ) != null ){

            intent = intentApp
        }
        else if( intentWeb != null
            && intentWeb.resolveActivity( requireActivity().packageManager ) != null ){
